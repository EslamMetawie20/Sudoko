    ...
    if difficulty `notElem` [1, 2, 3]
        then do
            ...
            let numRemove = case difficulty of
                    ...
                    2 -> 40
                    _ -> 60
            maybeSolution <- generateSudoku
            case maybeSolution of
                Nothing -> putStrLn "Failed to generate a Sudoku solution. Exiting..."
                Just solution -> do
                    putStrLn "Puzzle will now be created."
                    --putStrLn "Solution grid (for debugging):"
                    --printGrid solution  -- Print the solution for debugging.
